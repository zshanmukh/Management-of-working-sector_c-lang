...
/*intialisationforinsertioninlinkedlist*/
  struct worker *temp;		/*temporaryvariablecreatedtoallocatethelocationofea
				   chworker */
  ...
  printf ("\n");
  scanf ("%d%s%d%s%s", &temp->id_no, &temp->name, &temp->amount, temp->sector,
	 temp->month);
  temp->link = NULL;
